...
class ImagesRepository(
    val apiService: PixabayApi = ApiManager.getInstance().create(PixabayApi::class.java)
) {

    fun letImagesFlow(pagingConfig: PagingConfig = getDefaultPageConfig()): Flow<PagingData<Images>> {
        return Pager(
            config = pagingConfig,
